...
//Copying out the directors and ratings
var rating = watchList.map((x) => {return {director: x.Director, rating: Number(x.imdbRating)};});
//Filtering out only object wherer Nolan is the director
var nolan = rating.filter((x) => {return x.director == "Christopher Nolan"});
console.log(nolan)
//Copying out just the rating numbers 
var nolanRatings = nolan.map((x) => x.rating);
console.log(nolanRatings);
//Using the reduce to sum
var averageRating = nolanRatings.reduce((total, amount) => (total + amount));
//Calculating average by dividing the sum by the number of ratings
averageRating = averageRating/nolanRatings.length;
